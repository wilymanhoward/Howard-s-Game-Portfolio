...
// Ensure we extend the fiber catalog with our custom material
// This is required for <dotMaterial /> to be valid
...
const DotMaterial = shaderMaterial(
    {
        ...
        dotColor: new THREE.Color('#0d59f2'), // Primary Blue
        bgColor: new THREE.Color('#0a0e17'), // Midnight Background
        mouseTrail: null,
        render: 0,
        rotation: 0,
        gridSize: 50,
...
function Scene() {
    ...
    // Safe use of useTheme, defaulting if provider missing
    let theme = 'dark'
    try {
        const themeCtx = useTheme()
        if (themeCtx.theme) theme = themeCtx.theme
    } catch (e) {
        // Provider likely missing, default to dark
    }
    ...
    const getThemeColors = () => {
        return {
            dotColor: '#0d59f2', // Primary Blue
            bgColor: '#0a0e17', // Midnight Background
            dotOpacity: 0.5
        }
    }
    ...
    // @ts-ignore - dotMaterial is custom
    ...
    return (
        <mesh scale={[scale, scale, 1]} onPointerMove={handlePointerMove}>
            ...
            {/* @ts-ignore - custom element from extend */}
            <dotMaterial
...
export const DotScreenShader = ({ eventSource }: { eventSource?: any }) => {
    return (
        <Canvas
            eventSource={eventSource || document.body}
            eventPrefix="client"
            gl={{
                antialias: true,
                ...
                alpha: true // Enable alpha so it can layer if needed
            }}
